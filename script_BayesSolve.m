...
if testingFlag
    dataParentDIR = 'test set';
else
    dataParentDIR = 'behavioural parameters';
end
dataParentDIR = uigetdir(dataParentDIR);
...
% for gtInd = GTRange % toggle this line with the one above to parallel computing
    if ~createHandlesOnly && seedRndGenFlag
        ...
        % threefry ensures that workers in parallel computing and client in
        % matlab session use the same generator and seed (parfor vs for)
...
for ss = GTRange
    ...
    for cc = globalFitInds
        ...
        nParamsTemp = numel(pIndsTemp);
        paramsTitlesTemp = paramsTitles(pIndsTemp);
        dependentParams = pIndsTemp == pIndsTemp';
        dependParamsTitlesTemp = cell(3, nParamsTemp);
        dependParamsTitlesTemp(:) = {''};
        for ii = 1:nParamsTemp
            if sum(dependentParams(ii, :)) > 1
                temp = find(dependentParams(ii, :));
                dependParamsTitlesTemp(1, temp) = {' ('};
                dependParamsTitlesTemp(2, temp) = intFolders;
                dependParamsTitlesTemp(3, temp) = {')'};
            end
        end
        paramsTitlesTemp = arrayfun(@(x) ...
            [paramsTitlesTemp{x}, ' ', dependParamsTitlesTemp{:, x}], ...
            1:nParamsTemp, 'UniformOutput', false);
        if ~loadedFlag(ss, 1)
            switch cc
                case 1
                    paramsTitlesTemp([1, 2]) = [];
                case 2
                    paramsTitlesTemp(2) = [];
                case 3
                    paramsTitlesTemp(1) = [];
        ...
        fid = fopen([saveDIR, 'DDM ', strrep(globalTitles{1+cc}, ':', ''), ...
            ' ', sources{ss}, '.txt'], 'w');
        fprintf(fid, '%s\t', paramsTitlesTemp{:});
        fprintf(fid, '\n');
